...
int n_points_for_sine = 20;
int n_points_for_circle = 20;
const float g = -0.25;
...
void drawRiver() {
    ...
    float stepSize = range / (float)n_points_for_sine;
    float x, y;
    ...
    for (int i = 0; i <= n_points_for_sine; i++){
        x = i * stepSize + left;
...
void drawCircle(float x, float y, float radius) {
    if (filled)
        ...
        glBegin(GL_LINE_LOOP);
    for (int i = 0; i <= n_points_for_circle; i++) {
        float theta = i / (float)n_points_for_circle * 2.0 * M_PI;
...
void myinit (void)
{
    ...
    //0.025
    float spacing = 0.75 / (n_logs + 1);
    float x = spacing;
    for (int i = 0; i < n_logs; i++) {
        logs[i] = Log { {x, 0}, 0.025 };
        x += spacing;
    }
}
...
void keyboardCB(unsigned char key, int x, int y)
{
    switch (key) {
        //-
        case 'l':
            if (n_points_for_sine > 4)
                n_points_for_sine--;
            if (n_points_for_circle > 4)
                n_points_for_circle--;
            break;
        case 'm':
            if (n_points_for_sine >= 4)
